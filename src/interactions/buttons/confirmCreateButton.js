...
/** Confirmacao final: dispara a criacao completa da guilda. */
export default {
  ...
  async execute(interaction, { services }) {
    ...
    if (!draft) {
      return replyEphemeral(interaction, {
        embeds: [errorEmbed('Seu rascunho expirou. Clique em **Criar Guilda** novamente.')],
        components: [],
      });
    }
    ...
    return replyEphemeral(interaction, {
      embeds: [
        successEmbed(
          `Guilda ${guild.name} [${guild.tag}] criada!`,
          [
            `Cargo: <@&${guild.roleId}>`,
            ...
            '',
            'Convide membros com `/guild invite @usuario`.',
          ].join('\n'),
        ),
      ],
      components: [],
